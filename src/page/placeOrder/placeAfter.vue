  ...
  <div>
    ...
    <div class="place-order-wrap">
      <div class="full-height">
        ...
        <CartItemListAfter></CartItemListAfter>
        <!--底部 小项列表-->
        <div class="bottom-button bottom-box">
          <div class="weui-cells page__category-content ">
            ...
            <a class="weui-cell weui-cell_access js_item" href="javascript:;">
                <div class="weui-cell__bd">
                    <p>实付金额 <span class="origin-price">原价&nbsp;&yen;{{totalPriceOrigin.toFixed(2)}}</span></p>
                    <p class="final-price"><span v-if="memberActivityByTelephone.discount" class="weui-badge badge-box" style="margin-left: 5px;">{{memberActivityByTelephone.discount}}折</span>&yen;{{pay.toFixed(2)}}</p>
                </div>
                <div class="weui-cell__ft"></div>
            </a>
          </div>
...
import CartItemListAfter from '../../components/cartControl/cartItemListAfter.vue'
...
export default {
  ...
  methods:{
    ...
    async setTableNumber() {
      ...
      let res = await customerApi.lastOrderByCustomer($.param(params));
      this.parentOrderId = res.data && res.data.id; 
      // alert(this.parentOrderId)     
      if (this.parentOrderId ) { // ‘继续点单’有父订单信息
        await this.setLastCartItem(this.parentOrderId);
        this.alterOrderParams({tableNumber: this.lastOrder.tableNumber, parentOrderId: this.lastOrder.parentOrderId || this.lastOrder.id}); // 设置桌号
      } else {
        this.alterOrderParams({tableNumber: null, parentOrderId: null});
      }
    },
