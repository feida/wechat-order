      ...
      <div  class="full-height" >
        ...
        <div class="scroll-wrap" v-iscroll=getIscroll style="height:284px; overflow:hidden;">
          <div style="padding: 0 10px;" >
            <!-- 重量包 名称 -->
            <CartItemCell :item="currentArticle" :currentArticle="currentArticle" ></CartItemCell>
            <!-- 重量包规格 -->
            <ArticleWeightUnit v-if="currentArticle.weightListChoose" :currentArticle="currentArticle" @selectWeight="selectWeight"></ArticleWeightUnit>
            <!-- 推荐配餐 -->
        ...
        <div class="bottom-button">
          <div class="weui-dialog__ft margin-top-0 no-border-top">
...
<script>
  ...
  import ArticleWeightUnit from './common/article-weight-unit.vue'
  import { getRecommendImgList, checkRecommendCount, autoScroll, calcPrice } from '../articles.js'
  ...
  export default {
    ...
    computed: {
      ...
      totalPrice() {
        return calcPrice(this.currentArticle);
      }
    },
    created() {
    },
    methods: {
      ...
      joinShopCart() {
        let flag = false;
        this.currentArticle.weightListChoose.details.forEach(u => {
          if (u.click && u.click===true) {
            flag = true;
          }
        });
        if (!flag) {
